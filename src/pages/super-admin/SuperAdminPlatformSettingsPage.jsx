...
const SETTINGS_METADATA = [
  {
    ...
    description: 'Let prospective teachers create their own accounts to await approval.',
    type: 'boolean',
  },
  {
    key: 'autoApproveStudents',
    label: 'Auto approve students',
    description: 'Automatically activate new student accounts without manual review.',
    type: 'boolean',
  },
  {
    key: 'sessionTimeoutMinutes',
    label: 'Session timeout (minutes)',
    description: 'Controls how long a user can stay signed in without activity.',
    type: 'number',
    min: 5,
    max: 480,
    step: 5,
  },
  {
    ...
    description: 'Trigger low balance alerts when credits fall below this amount.',
    type: 'number',
    min: 0,
    max: 10000,
    step: 10,
...
const SuperAdminPlatformSettingsPage = () => {
  ...
  const dirtyDiff = useMemo(() => {
    ...
    return Object.entries(formValues).reduce((acc, [key, value]) => {
      if (key === 'updatedAt' || key === 'updatedById' || key === 'updatedByName') {
        return acc;
      }
      ...
      return (
        <label key={config.key} htmlFor={id} className="flex items-start justify-between gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
          ...
          <input
            ...
            onChange={(event) => handleFieldChange(config.key, event.target.checked)}
          />
        </label>
      );
    }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Fine-tune onboarding, security, and operational preferences across the entire platform.
        </p>
      </div>
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Changes apply worldwide immediately after saving.</p>
            </div>
            {initialValues?.updatedAt ? (
              <div className="text-xs text-gray-500">
                <p>Last updated: {new Date(initialValues.updatedAt).toLocaleString()}</p>
                <p>
                  By:{' '}
                  <span className="font-medium text-gray-700">
                    {initialValues.updatedByName ?? initialValues.updatedById ?? 'Unknown'}
                  </span>
                </p>
              </div>
            ) : null}
          ...
          <form onSubmit={handleSubmit} className="px-6 py-6 space-y-6">
            ...
            <div className="flex flex-col gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center space-x-3">
                {successMessage ? <span className="text-sm text-green-600">{successMessage}</span> : null}
                <button
                  type="submit"
                  disabled={saving}
