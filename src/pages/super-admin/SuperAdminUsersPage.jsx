...
const SuperAdminUsersPage = () => {
  ...
  }, [role, status, debouncedSearch, page, pageSize, refreshFlag]);
  ...
  const statusSummary = useMemo(() => {
    ...
    return [
      { label: 'Approved', value: summary.approved ?? 0, tone: 'text-green-600' },
      { label: 'Pending', value: summary.pending ?? 0, tone: 'text-yellow-600' },
      { label: 'Rejected', value: summary.rejected ?? 0, tone: 'text-red-600' },
      { label: 'Inactive', value: summary.inactive ?? 0, tone: 'text-gray-600' },
    ];
  }, [summary]);
  ...
  const handleSubmitAction = async () => {
    ...
    try {
      ...
      const message =
        err?.response?.data?.message ?? err?.message ?? 'Unable to update user status.';
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Full visibility into teachers, students, and admin accounts with live filters and approval controls.
        </p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-gray-100 bg-white px-5 py-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-semibold text-gray-800">{numberFormatter.format(meta?.total ?? 0)}</p>
        </div>
        {statusSummary.map((item) => (
          <div key={item.label} className="rounded-lg border border-gray-100 bg-white px-5 py-4 shadow-sm">
            <p className="text-xs uppercase tracking-wide text-gray-500">{item.label}</p>
            <p className={`mt-2 text-2xl font-semibold ${item.tone}`}>{numberFormatter.format(item.value)}</p>
          </div>
        ))}
      </div>
        ...
        <div className="border-b border-gray-100 px-6 py-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Filter by role or approval state to take action quickly.</p>
            </div>
                    ...
                    data.map((user) => {
                      ...
                      return (
                        <tr key={user.id} className="hover:bg-gray-50">
                          <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{displayName}</td>
                          <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{formatRole(user.role)}</td>
                          <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{user.email}</td>
                          <td className="whitespace-nowrap px-6 py-4 text-sm">{renderStatus(user.status)}</td>
                          <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{user.phoneNumber ?? 'N/A'}</td>
                          ...
                          <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                            {isSuperAdmin ? (
                              <span className="text-xs uppercase tracking-wide text-gray-400">Protected</span>
                            ) : (
