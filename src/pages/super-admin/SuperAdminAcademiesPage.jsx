...
const AcademySummaryCard = ({ label, value, accent }) => (
  <div className="rounded-lg border border-gray-100 bg-white px-5 py-4 shadow-sm">
    ...
    <p className="mt-2 text-2xl font-semibold text-gray-800">
      ...
      {accent ? <span className="ml-2 text-sm font-medium text-gray-500">{accent}</span> : null}
    </p>
  </div>
);
...
const SuperAdminAcademiesPage = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-green-800">Academy Management</h1>
        <p className="mt-2 text-gray-600">
          Track academy onboarding, approval status, and platform utilisation in real time.
        </p>
      </div>

      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <AcademySummaryCard label="Total academies" value={totalAcademies} accent={summary ? `${numberFormatter.format(summary.pending)} pending` : ''} />
        {statusSummary.map((item) => (
          <AcademySummaryCard key={item.label} label={item.label} value={item.value ?? 0} />
        ))}
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="rounded-lg border border-gray-100 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-6 py-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Academy Directory</h2>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Owner</th>
                    ...
                    data.map((academy) => (
                      <tr key={academy.id} className="hover:bg-gray-50">
                        <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">
                          {[academy.firstName, academy.lastName].filter(Boolean).join(' ') || academy.email}
                        </td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{academy.email}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm">{renderStatusBadge(academy.status)}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{academy.phoneNumber ?? '—'}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                          {academy.createdAt ? new Date(academy.createdAt).toLocaleDateString() : '—'}
                        </td>
                      </tr>
                    ))
                  )}
